...
# Updated by Stefan Kuhn to be compatible with TensorFlow 2.x
def batch_pairwise_distances(U, V):
    """Compute pairwise distances between two batches of feature vectors."""
    # Squared norms of each row in U and V.
    norm_u = tf.reduce_sum(tf.square(U), 1)
    ...
    # Pairwise squared Euclidean distances.
    D = tf.maximum(norm_u - 2*tf.matmul(U, V, transpose_b=True) + norm_v, 0.0)
    return D
...
class DistanceBlock:
    ...
    def __init__(self, num_features, num_gpus):
        self.num_features = num_features
        self.num_gpus = num_gpus
        # No placeholders are required, will use tensors directly

    def pairwise_distances(self, U, V):
        """Evaluate pairwise distances between two batches of feature vectors."""
        features_split2 = tf.split(V, self.num_gpus, axis=0)
        distances_split = []

        for gpu_idx in range(self.num_gpus):
            # Using tf.device for managing devices in TensorFlow 2.x
            with tf.device('/gpu:%d' % gpu_idx):
                distances_split.append(batch_pairwise_distances(U, features_split2[gpu_idx]))

        distance_block = tf.concat(distances_split, axis=1)
...
def knn_precision_recall_features(ref_features, eval_features, nhood_sizes=[3],
    ...
    logger.debug('Evaluating k-NN precision and recall with %i samples...' % num_images)
    start = time()
    ...
    logger.debug('Evaluated k-NN precision and recall in: %gs' % (time() - start))
